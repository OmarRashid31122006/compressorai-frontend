...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#080e1a] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 glass-dark border-b border-cyan-400/10">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <div className="w-9 h-9 rounded-xl border border-yellow-400/40 bg-yellow-400/10 flex items-center justify-center animate-glow-yellow flex-shrink-0">
              <Zap className="w-5 h-5 text-yellow-400" />
            </div>
            <div>
              <span className="font-display font-800 text-white text-base md:text-lg">CompressorAI</span>
              <div className="text-yellow-400/60 text-[8px] md:text-[9px] font-mono -mt-0.5 tracking-widest">INDUSTRIAL OPTIMIZER</div>
            </div>
          </div>
          {/* Nav buttons */}
          <div className="flex items-center gap-1 md:gap-2">
            <button onClick={() => navigate('/tutorial')}
              className="hidden sm:block text-slate-400 hover:text-white transition-colors text-sm font-display px-3 py-2 hover:bg-white/5 rounded-lg">
              Tutorial
            </button>
            <button onClick={() => navigate('/login')}
              className="hidden sm:flex btn-ghost text-sm py-2 px-4">Login</button>
            <button onClick={() => navigate('/register')}
              className="btn-primary text-sm py-2 px-4 md:px-5">Get Started</button>
          </div>
        </div>
      </nav>

      {/* ══════════════════════════════════════
          HERO — DESKTOP (lg+): side by side
      ══════════════════════════════════════ */}
      <section className="hidden lg:flex items-center relative overflow-hidden"
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 grid grid-cols-2 gap-8 items-center"
          ...
          <motion.div initial={{ opacity:0, x:-50 }} animate={{ opacity:1, x:0 }}
            ...
            <h1 className="font-display font-900 leading-tight mb-4">
            ...
            <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.6 }}
              className="text-slate-400 text-sm leading-relaxed mb-5 max-w-lg">
          ...
          <motion.div
            ...
            <div className="relative rounded-2xl overflow-hidden"
              ...
              <div className="absolute top-3 left-3 w-5 h-5 pointer-events-none" style={{borderTop:'1px solid rgba(250,204,21,0.4)',borderLeft:'1px solid rgba(250,204,21,0.4)'}}/>
              <div className="absolute top-3 right-3 w-5 h-5 pointer-events-none" style={{borderTop:'1px solid rgba(250,204,21,0.4)',borderRight:'1px solid rgba(250,204,21,0.4)'}}/>
              <div className="absolute bottom-3 left-3 w-5 h-5 pointer-events-none" style={{borderBottom:'1px solid rgba(250,204,21,0.4)',borderLeft:'1px solid rgba(250,204,21,0.4)'}}/>
              <div className="absolute bottom-3 right-3 w-5 h-5 pointer-events-none" style={{borderBottom:'1px solid rgba(250,204,21,0.4)',borderRight:'1px solid rgba(250,204,21,0.4)'}}/>
            </div>
          </motion.div>
        </div>
      ...
      {/* ══════════════════════════════════════
          HERO — MOBILE (< lg): stacked
      ══════════════════════════════════════ */}
      <section className="lg:hidden relative pt-16"
        style={{ minHeight:'100svh' }}>
        <div className="absolute inset-0 bg-grid opacity-40" />

        <div className="relative z-10 px-5 pt-6 pb-4 flex flex-col"
          style={{ minHeight:'calc(100svh - 64px)' }}>

          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-3 w-fit"
            style={{ border:'1px solid rgba(250,204,21,0.3)', background:'rgba(250,204,21,0.06)' }}>
            <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse" />
          ...
          <h1 className="font-display font-900 leading-tight mb-3">
            <span className="block text-white text-4xl">Industrial</span>
            <span className="block gradient-text-yellow text-4xl neon-yellow">Air Compressor</span>
            <span className="block text-white text-4xl">Optimizer</span>
          </h1>

          {/* Description */}
          <p className="text-slate-400 text-sm leading-relaxed mb-4">
            Upload compressor data — our AI pipeline finds
            <span className="text-yellow-400"> optimal operating parameters</span> for
            maximum efficiency at minimum electrical cost.
          </p>

          {/* Buttons — full width on mobile */}
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <button onClick={handleStart}
              className="btn-primary flex items-center justify-center gap-2 text-sm py-3.5 w-full sm:w-auto sm:px-8">
              Start Optimizing <ChevronRight size={16} />
            </button>
            <button onClick={() => navigate('/tutorial')}
              className="btn-ghost-yellow flex items-center justify-center gap-2 text-sm py-3.5 w-full sm:w-auto sm:px-8">
              Watch Tutorial <ArrowRight size={14} />
            </button>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-4 gap-2 pb-4 border-b border-white/5 mb-4">
            {[
              ...
              <div key={label} className="text-center">
                <div className="font-display font-900 text-base text-yellow-400">{val}</div>
                <div className="text-[9px] text-slate-500 font-mono mt-0.5">{label}</div>
              </div>
            ))}
          </div>

          {/* 3D Model — takes remaining space, min 300px */}
          <div className="flex-1 rounded-2xl overflow-hidden relative"
            style={{
              minHeight: '300px',
              background: 'radial-gradient(ellipse at center, rgba(8,20,40,0.7) 0%, rgba(4,10,20,0.98) 100%)',
            ...
            <div className="absolute top-2 left-2 w-4 h-4 pointer-events-none" style={{borderTop:'1px solid rgba(250,204,21,0.5)',borderLeft:'1px solid rgba(250,204,21,0.5)'}}/>
            <div className="absolute top-2 right-2 w-4 h-4 pointer-events-none" style={{borderTop:'1px solid rgba(250,204,21,0.5)',borderRight:'1px solid rgba(250,204,21,0.5)'}}/>
            <div className="absolute bottom-2 left-2 w-4 h-4 pointer-events-none" style={{borderBottom:'1px solid rgba(250,204,21,0.5)',borderLeft:'1px solid rgba(250,204,21,0.5)'}}/>
            <div className="absolute bottom-2 right-2 w-4 h-4 pointer-events-none" style={{borderBottom:'1px solid rgba(250,204,21,0.5)',borderRight:'1px solid rgba(250,204,21,0.5)'}}/>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 relative">
          <motion.div className="text-center mb-12 md:mb-16"
            ...
            <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-800 text-white mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            {features.map(({ icon: Icon, title, desc, color }, i) => (
              <motion.div key={title}
                ...
                <h3 className={`font-display font-700 text-base md:text-lg mb-2 ${
                  color==='yellow' ? 'text-yellow-400' : color==='cyan' ? 'text-cyan-400' : 'text-white'
                }`}>{title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            {steps.map(({ step, title, desc, color }, i) => (
              <motion.div key={step}
                ...
                <div className={`text-7xl md:text-8xl font-display font-900 mb-3 ${
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6 text-center">
          <motion.div initial={{ opacity:0, scale:0.9 }} whileInView={{ opacity:1, scale:1 }} viewport={{ once:true }}>
            <div className="relative rounded-3xl p-[1px] overflow-hidden">
              ...
              <div className="relative rounded-3xl p-8 md:p-12" style={{ background:'rgba(8,14,26,0.95)' }}>
      ...
      <footer className="border-t border-white/5 py-8">
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-slate-600 text-sm font-mono text-center md:text-left">
